...
static int has_1ghz_support(void)
{
	static int cached_st=0;
	...
	if (!cached_st) {
		...
		/* enable peripheral domain clock source */
		sr32(CM_CLKSEL_PER, 0, 1, 0x1); /* GPT2 = sys clk */
		sr32(CM_ICLKEN_PER, 3, 1, 0x1); /* ICKen GPT2 */
		sr32(CM_FCLKEN_PER, 3, 1, 0x1); /* FCKen GPT2 */

		/* prepare mux for GPIO38 and GPIO112 */
		MUX_VAL(CP(GPMC_A5), (IEN | PTD | DIS | M4) );
		MUX_VAL(CP(CSI2_DX0), (IEN | PTD | DIS | M4) );
		...
		hwid2 = (gpio2_base->datain & (1<<6)) >> 6;
		hwid1 = (gpio4_base->datain & (1<<16)) >> 16;

		cached_st = ((__raw_readl(CONTROL_SCALABLE_OMAP_STATUS) & 1) != 1) && (hwid1 || hwid2);
