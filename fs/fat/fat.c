...
#define DOS_PART_TBL_OFFSET	0x1be	// first entry in partition table
#define DOS_PART_TBL_P1_LBA	(DOS_PART_TBL_OFFSET + 8) // disk offset of first partition
...
fat_register_device(block_dev_desc_t *dev_desc, int part_no)
{
    ...
    (CONFIG_COMMANDS & CFG_CMD_USB) || (CONFIG_COMMANDS & CFG_CMD_MMC) || defined(CONFIG_SYSTEMACE)
		 ...
		 * A simpler solution is to use the offset of the first
		 * partition slot in the MBR.
		 */
		part_offset=	(buffer[DOS_PART_TBL_P1_LBA+0] << 0)  |
				(buffer[DOS_PART_TBL_P1_LBA+1] << 8)  |
				(buffer[DOS_PART_TBL_P1_LBA+2] << 16) |
				(buffer[DOS_PART_TBL_P1_LBA+3] << 24);
		//printf("part_offset %d\n", part_offset);
		cur_part = 1;
#endif
	}
	return 0;
...
/*
 * Plonk a bunch of stuff into DRAM - the x-loader is executing out of 64KB of SRAM.
 * and we don't have space for these variables in there.
 */

__u8 *fnamecopy = 0x80500000;
__u8 *do_fat_read_block = 0x80500880;
#ifdef CONFIG_SUPPORT_VFAT
__u8 *get_vfatname_block = 0x80510880;
/*
	     ...
	     int dols)
{
    ...
    FAT_DPRINT ("FAT%d, fat_sect: %d, fatlength: %d\n",
               ...
               mydata->fat_sect,
               mydata->fatlength);
    FAT_DPRINT ("Rootdir begins at cluster: %d, sector: %u, offset: %x\n"
	...
	int i;
       ...
       FAT_DPRINT ("FAT read sect=%d, clust_size=%d, dirents_per_block=%d\n",
	...
	for (i = 0; i < dirents_per_block; i++) {
       ...
       if (mydata->fatsize == 32) {
               ...
               FAT_DPRINT ("END LOOP: sect=%d, clust=%d, n_sect=%d, n_clust=%d\n",
